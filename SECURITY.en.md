...
Configuration and connection keys are stored under `/opt/snellctl/generations/`. The `snellctl export` command prints the connection key, so keep its output private. Default uninstall retains this data; use `uninstall --purge` for full removal.
